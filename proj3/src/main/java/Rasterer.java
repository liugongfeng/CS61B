...
public class Rasterer {
    ...
    private int[] getXY() {
        ...
        double x1 = (ullon - MapServer.ROOT_ULLON) / (MapServer.ROOT_LRLON - MapServer.ROOT_ULLON);
        for (int i = 0; i <= N; i++) {
            if (x1 <= 0) {
                xy[0] = 0;
                break;
        ...
        double y1 = (ullat - MapServer.ROOT_LRLAT) / (MapServer.ROOT_LRLAT - MapServer.ROOT_ULLAT);
        for (int i = 0; i <= N; i++) {
            if (y1 <= 0) {
                xy[1] = 0;
                break;
        ...
        double x2 = (lrlon - MapServer.ROOT_ULLON) / (MapServer.ROOT_LRLON - MapServer.ROOT_ULLON);
        if (x2 >= 1) {
        ...
        double y2 = (lrlat - MapServer.ROOT_ULLAT) / (MapServer.ROOT_LRLAT - MapServer.ROOT_ULLAT);
        if (y2 >= 1) {
